...
Tidak ada login username/password untuk **operasional harian** — transaksi (sale/void/settlement/dst) berjalan tanpa cashier login sama sekali, device diautentikasi ke Front-facing API via **digital signature request-signing**. **Provisioning (J1)** juga **tidak** butuh login di device: otorisasinya sepenuhnya datang dari `challengeCode` hasil scan QR yang diterbitkan Cashup backoffice, dan admin login di backoffice itu — sistem terpisah — bukan di terminal (lihat §2 dan §4 J1). **Re-provisioning (J2) dan deactivation (J3) di luar scope spec provisioning ini** (lihat `docs/superpowers/specs/2026-09-16-provisioning-design.md` §10) — bagian dokumen ini yang masih menyebut login teknisi untuk J2/J3 (§4 J2/J3, §11) mencerminkan desain lama yang belum direvisi ulang untuk dua journey itu; jangan disamakan dengan J1 sampai itu ditinjau. App ini **fully stateless** — tidak ada local database; semua histori/reprint/settlement query live ke Front-facing API (CDCP+QRIS).
